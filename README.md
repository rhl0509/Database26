...
MySql 설치를 진행한다.
https://dev.mysql.com/downloads/mtsql (LTS 버전을 선택한다.)

mysql 설치 중 오류가 발생하면 vs code 라이브러리 설치
구글 검색 : Visula C++ Redistributable v14(vc_redist.x64.exe) 필요

mysql 관련 프로그램을 설치한다.
https://dev.mysql.com/downloads/workbench
```
데이터베이스 : 자료를 24시간 365동안 보관하며 서비스를 제공
DBMS : 데이터베이스 프로그램 (오라클 / MySql / MARIADB / SQLITE ....)
SQL : Structured(구조화된) Query(질의) Language(언어) 표준화된 문법
정의어 : (DDL : Data Definituin Language)
		...
		-> 이름 변경 : RENAME TABLE / RENAME USER
		-> 읽기 : SELECT
조작어 : (DML : Data Manipulation Language)
		-> 데이터베이스 테이블에 자료 관리용
		-> C(회원가입) : INSERT INTO 테이블명(필드명들)  VALUES  (값들)
...
제어어 : DCL : Data Control Language)
		-> 데이터 보안, 무결성, 데이터회복, 병행수행 등 ...(데이터관리목적)
		-> ROLLBACK : 트렌젝션 복귀
		-> COMMIT : 트렌젝션 저장
				(트렌젝션 : 일괄작업)
